...
function min_btw_phi_n_K(phi ::phi, T ::T, x ::Union{Float64, Interval{Float64}, BigFloat, Interval{BigFloat}}, s ::Union{Float64, Interval{Float64}, BigFloat, Interval{BigFloat}}, n ::Int64, K ::Union{Float64, BigFloat}) ::Union{Float64, Interval{Float64}, BigFloat, Interval{BigFloat}}
    "Compute recursevely the value of the minimum between phi^{(n)} and K."
    if n == 0
        return s
    end
    return phi.value(x, min(min_btw_phi_n_K(phi, T, T.value(x), s, n-1, K), K))
end
...
function find_K(phi ::phi, T ::T, N ::Int64, coeff ::Float64, K_max ::Float64, iteration ::Int64) ::Union{Float64,Bool}
...
function F_upper(inter ::Union{Interval{Float64}, Interval{BigFloat}}, T ::T, m ::Int64, K ::Union{Float64, BigFloat}, phi ::phi) ::Union{Interval{Float64}, Interval{BigFloat}}
...
function upper_bound_lambda_f(phi ::phi, T ::T, N ::Int64, iteration ::Int64, N_find_K ::Int64, K_max ::Float64, coeff ::Float64, iteration_find_K ::Int64, big_float ::Union{Nothing, Int64}) ::Float64
    "This function allow us to find a good upper bound on lambda_f"
    if big_float == nothing
        l = [str_interval(interval((i-1)/2^N, i/2^N), N, true, 1000.0) for i in 1:2^N]
        K = find_K(phi, T, N_find_K, coeff, K_max, iteration_find_K)
    else
    ...
    for _ in 1:iteration
        l = compute_of_bounds(T, l, F_upper, K, phi)
...
function log_derivate_T_upper(inter ::Union{Interval{Float64}, Interval{BigFloat}}, T ::T, m, K, phi) ::Union{Interval{Float64}, Interval{BigFloat}}
    "This function is the one whose Birkhoff sum can be used to bound lambda_f."
    return log(T.derivate(inter))
end

function upper_bound_lambda_u(T ::T, N ::Int64, iteration ::Int64, big_float ::Union{Nothing, Int64}) ::Float64
    ...
    for _ in 1:iteration 
        l = compute_of_bounds(T, l, log_derivate_T_upper)
        l = split_worst_intervals(l, 0.95)
    end
    return maximum([I.bound for I in l])
end
